...
function Product() {
  ...
  return (
    <div className="flex items-center justify-center flex-col gap-y-3 sm:gap-y-6 md:gap-y-10 w-full">
      <Title>جزئیات محصول</Title>
      {isLoading ? (
        <Spinner />
      ) : (
        <>
          <div className=" w-full md:w-[700px] md:h-[400px] md:rounded-lg md:shadow-Normul mx-auto flex items-center justify-start sm:flex-row flex-col-reverse p-7 ">
            <div className=" w-[95%] sm:w-[50%] flex items-start flex-col gap-y-1 md:gap-y-2">
              <h3 className=" font-DanaBold text-base md:text-xl">
                ...
                <span>{data[0].nameProduct}</span>
              </h3>
              <p className=" font-DanaMedium text-xs md:text-base sm:text-right text-center">
                <span className=" text-bg1">دسته بندی:</span>{" "}
                <span>
                  {data[0].Category === "MainCourse" && "غذای اصلی"}
                  {data[0].Category === "Drink" && "نوشیدنی"}
                  {data[0].Category === "sauce" && "انواع سس"}
                </span>
              </p>
              ...
              <p className=" child:font-DanaMedium text-xs child:md:text-base">
                ...
                <span className=" text-bg1">{data[0].price}</span>
              </p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      dispatch({
                        ...
                        payload: { idProduct: data[0].id, quantityProduct: 1 },
                      });
                    }}
                  >
                    +
            ...
            <div className="w-full flex justify-center sm:w-[50%]">
              <img
                ...
                src={data[0].image}
                alt={data[0].nameProduct}
              />
            </div>
          </div>
          {data[0].Category == "MainCourse" && (
            <>
              <h2 className=" font-DanaBold font-bold text-2xl">پیشنهادات</h2>
              <Suggestions />
            </>
          )}
        </>
